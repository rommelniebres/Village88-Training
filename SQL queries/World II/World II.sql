...
SELECT continent, count(*) AS total_countries, LifeExpectancy FROM country WHERE LifeExpectancy>70 GROUP BY continent;

-- 2. How many countries in each continent have life expectancy between 60 and 70?
SELECT continent, count(*) AS total_countries, LifeExpectancy FROM country WHERE LifeExpectancy BETWEEN 60 AND 70 GROUP BY continent;

-- 3. How many countries have life expectancy greater than 75?
SELECT name AS country, LifeExpectancy FROM country WHERE LifeExpectancy > 75;

-- 4. How many countries have life expectancy less than 40?
SELECT name AS country, LifeExpectancy FROM country WHERE LifeExpectancy < 40;

-- 5. How many people live in the top 10 countries with the most population?
SELECT name AS country, Population FROM country GROUP BY population DESC;
...
SELECT continent, sum(Population) as total_population FROM country GROUP BY continent HAVING sum(Population) > 500000000;
...
SELECT continent, COUNT(Name) AS country, sum(Population) AS total_population, AVG(LifeExpectancy) AS life_expectancy FROM country GROUP BY continent HAVING avg(LifeExpectancy) < 71;
...
SELECT country.name as country, countrylanguage.language, count(country.name) AS number_of_countries
FROM country
...
SELECT countrylanguage.Language as language, count(country.name) AS total_countries, countrylanguage.IsOfficial
FROM country
INNER JOIN countrylanguage
ON country.Code = countrylanguage.CountryCode
...
SELECT countrylanguage.Language as language, sum(country.population * countrylanguage.Percentage) AS total_population
FROM country
INNER JOIN countrylanguage
ON country.Code = countrylanguage.CountryCode
GROUP BY countrylanguage.Language;